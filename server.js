...
// ── Attach screenshots to specific test cases ─────────────
// POST /attach-screenshots
// Body: { testpadKey, projectId, scriptId, screenshots: [{testId, dataUrl, filename}] }
app.post('/attach-screenshots', async function (req, res) {
  try {
    var body       = req.body;
    var testpadKey = cleanKey(body.testpadKey);
    var projectId  = body.projectId;
    var scriptId   = body.scriptId;
    var screenshots = body.screenshots || [];

    if (!testpadKey || !projectId || !scriptId) {
      return res.status(400).json({ error: 'Missing testpadKey, projectId, or scriptId' });
    }
    ...
    for (var i = 0; i < screenshots.length; i++) {
      ...
      if (!shot.dataUrl || !shot.testId) continue;

      try {
        var base64   = shot.dataUrl.replace(/^data:image\/\w+;base64,/, '');
        var buffer   = Buffer.from(base64, 'base64');
        var filename = shot.filename || ('screenshot_' + (i + 1) + '.jpg');
        var attachUrl = baseUrl + '/projects/' + projectId +
                        '/scripts/' + scriptId +
                        '/tests/' + shot.testId + '/attachments';

        var r = await fetch(attachUrl, {
          method: 'POST',
          headers: {
            'Authorization':      'apikey ' + testpadKey,
            'Content-Type':       'image/jpeg',
        ...
        results.push({ testId: shot.testId, filename: filename, status: r.status });
        console.log('Attached screenshot to test', shot.testId, '- status:', r.status);
      } catch (attachErr) {
        results.push({ testId: shot.testId, error: attachErr.message });
      }
    }
